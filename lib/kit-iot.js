...
KitIoT.prototype.connect = function () {
  ...
  if (!this.arduino) {
    ...
    this.energy           = 0;
    this.lightLivingRoom  = new kit.LightEconomize({arduino: this.arduino, pin : 13}); 
    this.lightBathRoom    = new kit.LightEconomize({arduino: this.arduino, pin : 11});
    this.lightBedRoomOne  = new kit.LightEconomize({arduino: this.arduino, pin : 10});
    this.lightBedRoomTwo  = new kit.LightEconomize({arduino: this.arduino, pin : 9});
    this.lightKitchen     = new kit.LightEconomize({arduino: this.arduino, pin : 6});
    this.lightGarden      = new kit.LightEconomize({arduino: this.arduino, pin : 5});
    this.lightAisle       = new kit.LightEconomize({arduino: this.arduino, pin : 3});
    ...
    this.light.on('read', function (m) {
      self.light.value = m;
      self.energy.value = lightLivingRoom.getEnergyToLight(m);
    });
...
KitIoT.prototype.saveData = function (data) {
  var self    = this,
      ...
      rawBody = '|||unknown||chave|'+ data.button +'#|||temperature||temperatura|'+ data.temperature +'#|||illuminance||luminosidade|'+ data.light +'#|||presence||capacitivo|'+ data.capacitive +'#|||energy||energia|'+ data.energy,
